...
| Mode | Description |
|------|-------------|
| **Absolute Threshold** | Reject frames exceeding user-defined limits on any metric. For a better out-of-the-box experience, only FWHM and Eccentricity are enabled by default; other metrics (SNR, Stars, Noise, Median) can be manually enabled. |
...
- **Click-to-Open Image** --- Clicking any bar or data point in the measurements graph opens the corresponding image file directly in the PixInsight workspace. Double-clicking a row in the measurements table does the same.
- **Folder Quality Assessment** --- After each run, a composite session-level quality score (range 0–1) and letter grade (A/B/C/D/F) are computed from five weighted components: acceptance ratio (30%), mean per-frame quality score (30%), score consistency/CV (20%), FWHM component (10%), and SNR component (10%). Displayed in a dedicated color-coded panel in the dialog and printed in the console report and CSV.
- **Extended Statistics Panel** --- The statistics panel shows per-metric min/max/mean/median/σ/CV statistics separately for accepted and rejected frames, enabling direct comparison of the two populations.
- **Rejection Reason Tooltips** --- Hovering over the Status cell of any rejected row in the measurements table shows a tooltip listing all rejection reasons for that frame.
...
- Hover over any bar/dot to see the frame name, metric value, status, and **rejection reasons** in a tooltip
...
- Double-click any row to open the corresponding image file in the PixInsight workspace.
- Hovering over the Status column of a rejected row shows a tooltip with all rejection reasons.
...
|--------|---------|-------------|---------|
| FWHM Max | 5.0" | Above | Poor seeing, defocus |
| FWHM Min | 0.1" | Below | Suspicious data, hot pixels |
| Eccentricity | 0.75 | Above | Trailing, guiding errors |
| SNR (Disabled*) | 1.0 | Below | Low signal quality |
| Star Residual (Disabled*) | 0.20 | Above | Poor PSF fit, distortion |
| Star Count (Disabled*) | 10 | Below | Clouds, fog, dew |
| Noise (Disabled*) | 0.1 | Above | Excessive background noise |
| Median Min (Disabled*) | 0.0001 | Below | Failed exposure, shutter issue |
| Median Max (Disabled*) | 0.95 | Above | Saturation, light leak, LP |

*\* These filters are disabled by default to prevent accidental rejection of linear images. Enable them manually if needed.*
...
After rejection is applied, the engine computes a session-level quality score stored in
`SubframeSelectorPro_cache.json` and displayed in the **Folder Quality Assessment** panel
(right side of the dialog).

| Component | Weight | Description |
|-----------|--------|-------------|
| Acceptance Ratio | 30% | Fraction of analysed frames that passed rejection |
| Mean Frame Score | 30% | Mean per-frame quality score of accepted frames |
| Consistency | 20% | 1 minus the coefficient of variation of accepted scores |
| FWHM Component | 10% | Accepted mean FWHM normalised over the full set |
| SNR Component | 10% | Accepted mean SNR normalised over the full set |

**Letter grades:** A ≥ 90%, B ≥ 75%, C ≥ 60%, D ≥ 40%, F < 40%.

The grade is color-coded in the panel (green → A, yellow → C, red → F) and also appended
as a comment block at the end of the CSV export.
...
In addition to the 16 per-frame columns, a **Folder Quality Score summary block** is
appended at the end of the file as comment lines (`#`), including grade, overall score,
acceptance ratio, mean frame score, consistency, FWHM and SNR components.
...
| Upper Limit | 1.0 | 0–1.0 | Saturation upper limit for star detection |
