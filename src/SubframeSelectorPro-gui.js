// ****************************************************************************
// SubframeSelectorPro-gui.js - User Interface
// ****************************************************************************
//
...
//
// Features:
// - Dual-panel layout with live measurement graphs and results table
// - Click on graph bars/points opens the corresponding image in PixInsight
// - Folder quality score panel with letter grade and component breakdown
// - Extended statistics panel with per-metric distribution details
// - All previous UI features (3 rejection modes, PSF settings, i18n)
// ****************************************************************************
...
// SSPGraphControl
//
// Custom control rendering bar charts and scatter plots of per-frame metrics.
// Clicking on a bar or data point opens the corresponding image file in the
// PixInsight workspace.  Mouse-over highlights the element and displays a
// tooltip with the frame path and metric value.
// ============================================================================
function SSPGraphControl( parent )
{
   ...
   // ========================================================================
   // getMetricValue()
   // Returns the current metric value for a given result record.
   // ========================================================================
   ...
   // ========================================================================
   // getRange()
   // Computes the padded value range across all valid results.
   // ========================================================================
   ...
   // ========================================================================
   // formatValue()
   // Formats a numeric value with context-appropriate decimal places.
   // ========================================================================
   ...
   // ========================================================================
   // hitTestIndex()
   // Maps a pixel x-coordinate to the corresponding valid-result index.
   // Returns -1 when the coordinate falls outside the plot area.
   // ========================================================================
   this.hitTestIndex = function( x )
   {
   ...
   // ========================================================================
   // openFileAtIndex()
   // Attempts to open the image file corresponding to result index i inside
   // the PixInsight workspace.  Silently ignores failures (e.g. file moved).
   // ========================================================================
   this.openFileAtIndex = function( i )
   {
      ...
      try
      {
         ...
         {
            Console.writeln( "Opening image: " + r.filePath );
            var windows = ImageWindow.open( r.filePath );
            ...
            Console.warningln( "File not found: " + r.filePath );
      }
      catch ( e )
      {
         Console.warningln( "Error opening file: " + e.message );
      }
   };
      ...
      try
      {
         ...
         if ( this.graphType === GRAPH_BAR || this.graphType === GRAPH_WEIGHT )
         {
            ...
            for ( var i = 0; i < n; ++i )
            {
               ...
               var v    = this.getMetricValue( r );
               var frac = Math.max( 0, Math.min( 1, (v - yMin) / ySpan ) );
         ...
         // Hint text for click interaction
         g.font = new Font( "Helvetica", 7 );
         g.pen  = new Pen( 0xFF666666 );
         g.drawText( plotL + 5, plotB - 3, "Click to open image" );
      }
      catch ( e ) {}
      g.end();
   };
   ...
   // ========================================================================
   // onMouseMove()
   // Highlights the element under the cursor and updates the tooltip.
   // ========================================================================
   this.onMouseMove = function( x, y, buttons, modifiers )
   {
      if ( this.results.length === 0 ) return;

      var newHi = this.hitTestIndex( x );
      ...
      if ( newHi !== this.highlightIndex )
      {
         ...
         if ( newHi >= 0 && this.results[newHi] )
         {
            ...
            var statusStr = r.rejected ? "REJECTED" : "ACCEPTED";
            if ( r.rejected && r.rejectionReasons.length > 0 )
               statusStr += " (" + r.rejectionReasons.join(", ") + ")";
   ...
   // ========================================================================
   // onMousePress()
   // Opens the image file corresponding to the clicked graph element.
   // ========================================================================
   this.onMousePress = function( x, y, button, buttons, modifiers )
   {
      Console.writeln( "Graph clicked at x=" + x + " y=" + y );
      var idx = this.hitTestIndex( x );
      if ( idx >= 0 )
         this.openFileAtIndex( idx );
   };
...
function SSPDialog( params )
{
   ...
   // Compact widget factory helpers
   function mkL(p,text,w,a)
   {
      var l = new Label(p);
      l.text          = text;
      l.textAlignment = (a || TextAlign_Right) | TextAlign_VertCenter;
      if ( w > 0 ) l.setFixedWidth(w);
      return l;
   }
   function mkCB(p,ch,tt)
   {
      var c = new CheckBox(p);
      c.text    = "";
      c.checked = ch;
      c.setFixedWidth(COL_CHECK_W);
      if ( tt ) c.toolTip = tt;
      return c;
   }
   function mkSp(p,lb,lw,v,lo,hi,pr,ew,tt)
   {
      var n = new NumericControl(p);
      n.label.text = lb;
      if ( lw > 0 ) n.label.setFixedWidth(lw);
      else n.label.hide();
      n.setRange(lo,hi);
      n.setPrecision(pr);
      n.setValue(v);
      n.edit.setFixedWidth(ew);
      if ( tt ) n.toolTip = tt;
      return n;
   }
   ...
   {
      var g = new GetDirectoryDialog();
      g.caption     = _T("SELECT_INPUT_DIR");
      g.initialPath = params.inputDirectory;
      if ( g.execute() )
         { params.inputDirectory = g.directory; dlg.inputDirEdit.text = g.directory; }
   };
   ...
   this.inputRow.add(this.inputDirLabel);
   this.inputRow.add(this.inputDirEdit, 100);
   this.inputRow.add(this.inputDirBtn);

   this.rejRow = new HorizontalSizer;
   this.rejRow.spacing = 4;
   this.rejRow.add(this.rejLabel);
   this.rejRow.add(this.rejEdit);
   this.rejRow.addStretch();
   ...
   this.optGB.sizer.margin  = 6;
   this.optGB.sizer.spacing = 16;
   this.optGB.sizer.add(this.dryRunCB);
   this.optGB.sizer.add(this.filterCB);
   this.optGB.sizer.add(this.csvCB);
   this.optGB.sizer.add(this.cacheCB);
   ...
   this.spFocal = mkSp(this.scaleGB, _T("FOCAL_LENGTH"), COL_PLABEL_W,
                        params.overrideFocalLength, 0, 20000, 0, 60, "mm (0=auto)");
   this.spFocal.onValueUpdated = function(v){ params.overrideFocalLength = v; };

   this.spPixel = mkSp(this.scaleGB, _T("PIXEL_SIZE"), COL_PLABEL_W,
                        params.overridePixelSize, 0, 50, 2, 55, "\u00B5m (0=auto)");
   ...
   this.scaleGB.sizer.margin  = 6;
   this.scaleGB.sizer.spacing = 4;
   this.scaleGB.sizer.add(this.spFocal); gap(this.scaleGB.sizer);
   this.scaleGB.sizer.add(this.spPixel); gap(this.scaleGB.sizer);
   this.scaleGB.sizer.add(this.nativeCB);
   this.scaleGB.sizer.addStretch();

   // Rejection mode group
   ...
   this.modeGB.sizer.margin  = 6;
   this.modeGB.sizer.spacing = 20;
   this.modeGB.sizer.add(this.thrR);
   this.modeGB.sizer.add(this.pctR);
   this.modeGB.sizer.add(this.sigR);
   ...
   this.cbFwhm = mkCB(this.thrGB, params.useFWHM);
   this.cbFwhm.onCheck = function(c){ params.useFWHM = c; };
   this.lbFwhm = mkL(this.thrGB, _T("FWHM"), COL_LABEL_W);
   this.luMin  = mkL(this.thrGB, _T("MIN"), 24, TextAlign_Left);
   this.luMin.styleSheet = "QLabel{color:#888;}";
   this.spFMin = mkSp(this.thrGB, "", 0, params.fwhmMin, 0, 10, 2, 55);
   this.spFMin.onValueUpdated = function(v){ params.fwhmMin = v; };
   this.luMax  = mkL(this.thrGB, _T("MAX"), 28, TextAlign_Left);
   this.luMax.styleSheet = "QLabel{color:#888;}";
   this.spFMax = mkSp(this.thrGB, "", 0, params.fwhmMax, 0.1, 30, 2, 55);
   this.spFMax.onValueUpdated = function(v){ params.fwhmMax = v; };
   this.luU    = mkL(this.thrGB, _T("ARCSEC"), 0, TextAlign_Left);
   this.luU.styleSheet = "QLabel{color:#888;}";

   this.fwR = new HorizontalSizer; this.fwR.spacing = 4;
   this.fwR.add(this.cbFwhm); this.fwR.add(this.lbFwhm); this.fwR.add(this.luMin);
   this.fwR.add(this.spFMin); this.fwR.addSpacing(8); this.fwR.add(this.luMax);
   this.fwR.add(this.spFMax); this.fwR.addSpacing(4); this.fwR.add(this.luU);
   this.fwR.addStretch();

   this.cbEcc = mkCB(this.thrGB, params.useEccentricity);
   this.cbEcc.onCheck = function(c){ params.useEccentricity = c; };
   this.lbEcc = mkL(this.thrGB, _T("ECC_MAX"), COL_LABEL_W);
   this.spEcc = mkSp(this.thrGB, "", 0, params.eccentricityMax, 0, 1, 3, 55);
   this.spEcc.onValueUpdated = function(v){ params.eccentricityMax = v; };

   this.cbSnr = mkCB(this.thrGB, params.useSNR);
   this.cbSnr.onCheck = function(c){ params.useSNR = c; };
   this.lbSnr = mkL(this.thrGB, _T("SNR_MIN"), COL_LABEL_W);
   this.spSnr = mkSp(this.thrGB, "", 0, params.snrMin, 0, 1000, 1, 55);
   this.spSnr.onValueUpdated = function(v){ params.snrMin = v; };
   ...
   this.esR.add(this.cbSnr); this.esR.add(this.lbSnr); this.esR.add(this.spSnr);
   this.esR.addStretch();

   this.cbRes = mkCB(this.thrGB, params.useStarResidual);
   this.cbRes.onCheck = function(c){ params.useStarResidual = c; };
   this.lbRes = mkL(this.thrGB, _T("RESIDUAL"), COL_LABEL_W);
   this.spRes = mkSp(this.thrGB, "", 0, params.starResidualMax, 0, 1, 4, 65);
   this.spRes.onValueUpdated = function(v){ params.starResidualMax = v; };

   this.cbStar = mkCB(this.thrGB, params.useStars);
   this.cbStar.onCheck = function(c){ params.useStars = c; };
   this.lbStar = mkL(this.thrGB, _T("MIN_STARS"), COL_LABEL_W);
   this.spStar = mkSp(this.thrGB, "", 0, params.starsMin, 0, 10000, 0, 55);
   this.spStar.onValueUpdated = function(v){ params.starsMin = Math.round(v); };
   ...
   this.rsR.add(this.cbStar); this.rsR.add(this.lbStar); this.rsR.add(this.spStar);
   this.rsR.addStretch();

   this.cbNoise = mkCB(this.thrGB, params.useNoise);
   this.cbNoise.onCheck = function(c){ params.useNoise = c; };
   this.lbNoise = mkL(this.thrGB, _T("NOISE_MAX"), COL_LABEL_W);
   this.spNoise = mkSp(this.thrGB, "", 0, params.noiseMax, 0, 1, 6, 75);
   this.spNoise.onValueUpdated = function(v){ params.noiseMax = v; };

   this.cbMed = mkCB(this.thrGB, params.useMedian);
   this.cbMed.onCheck = function(c){ params.useMedian = c; };
   this.lbMed   = mkL(this.thrGB, _T("MEDIAN"), COL_LABEL_W);
   this.spMedMin = mkSp(this.thrGB, "", 0, params.medianMin, 0, 1, 4, 55);
   this.spMedMin.onValueUpdated = function(v){ params.medianMin = v; };
   this.lbDash  = mkL(this.thrGB, "--", 10, TextAlign_Center);
   this.spMedMax = mkSp(this.thrGB, "", 0, params.medianMax, 0, 1, 4, 55);
   this.spMedMax.onValueUpdated = function(v){ params.medianMax = v; };
   ...
   this.nmR.add(this.cbMed); this.nmR.add(this.lbMed);
   this.nmR.add(this.spMedMin); this.nmR.add(this.lbDash); this.nmR.add(this.spMedMax);
   this.nmR.addStretch();

   this.thrGB.sizer = new VerticalSizer;
   this.thrGB.sizer.margin  = 8;
   this.thrGB.sizer.spacing = 5;
   this.thrGB.sizer.add(this.fwR);
   this.thrGB.sizer.add(this.esR);
   this.thrGB.sizer.add(this.rsR);
   this.thrGB.sizer.add(this.nmR);

   // Percentage mode group
   ...
   this.keepSlider.setRange(1,100);
   this.keepSlider.setPrecision(0);
   this.keepSlider.slider.setRange(1,100);
   this.keepSlider.slider.minWidth = 200;
   ...
   this.kR = new HorizontalSizer;
   this.kR.add(this.keepSlider, 100);

   this.wLabel = new Label(this.pctGB);
   this.wLabel.text       = _T("WEIGHTS_TITLE");
   this.wLabel.styleSheet = "QLabel{font-weight:bold;}";

   this.wF = mkSp(this.pctGB, _T("W_FWHM"),  COL_WLABEL_W, params.weightFWHM,         0,100,0,42);
   this.wF.onValueUpdated = function(v){ params.weightFWHM = Math.round(v); };
   this.wE = mkSp(this.pctGB, _T("W_ECC"),   COL_WLABEL_W, params.weightEccentricity,  0,100,0,42);
   this.wE.onValueUpdated = function(v){ params.weightEccentricity = Math.round(v); };
   this.wS = mkSp(this.pctGB, _T("W_SNR"),   COL_WLABEL_W, params.weightSNR,            0,100,0,42);
   this.wS.onValueUpdated = function(v){ params.weightSNR = Math.round(v); };
   this.wT = mkSp(this.pctGB, _T("W_STARS"), COL_WLABEL_W, params.weightStars,          0,100,0,42);
   this.wT.onValueUpdated = function(v){ params.weightStars = Math.round(v); };
   this.wN = mkSp(this.pctGB, _T("W_NOISE"), COL_WLABEL_W, params.weightNoise,          0,100,0,42);
   this.wN.onValueUpdated = function(v){ params.weightNoise = Math.round(v); };

   this.w1 = new HorizontalSizer; this.w1.spacing = 4;
   this.w1.add(this.wF); gap(this.w1); this.w1.add(this.wE); this.w1.addStretch();
   this.w2 = new HorizontalSizer; this.w2.spacing = 4;
   this.w2.add(this.wS); gap(this.w2); this.w2.add(this.wT); this.w2.addStretch();
   this.w3 = new HorizontalSizer; this.w3.spacing = 4;
   this.w3.add(this.wN); this.w3.addStretch();

   this.pctGB.sizer = new VerticalSizer;
   this.pctGB.sizer.margin  = 8;
   this.pctGB.sizer.spacing = 5;
   this.pctGB.sizer.add(this.kR);
   this.pctGB.sizer.addSpacing(4);
   this.pctGB.sizer.add(this.wLabel);
   this.pctGB.sizer.add(this.w1);
   this.pctGB.sizer.add(this.w2);
   this.pctGB.sizer.add(this.w3);

   // Sigma mode group
   this.sigGB = new GroupBox(this);
   this.sigGB.title = _T("SIGMA_TITLE");

   this.spSL = mkSp(this.sigGB, _T("SIGMA_LOW"),  COL_SLABEL_W, params.sigmaLow,  0.5,10,1,50);
   this.spSL.onValueUpdated = function(v){ params.sigmaLow = v; };
   this.spSH = mkSp(this.sigGB, _T("SIGMA_HIGH"), COL_SLABEL_W, params.sigmaHigh, 0.5,10,1,50);
   this.spSH.onValueUpdated = function(v){ params.sigmaHigh = v; };

   this.ssR = new HorizontalSizer; this.ssR.spacing = 4;
   this.ssR.add(this.spSL); gap(this.ssR); this.ssR.add(this.spSH); this.ssR.addStretch();

   this.sigML = new Label(this.sigGB);
   this.sigML.text       = _T("SIGMA_APPLY_TO");
   this.sigML.styleSheet = "QLabel{font-weight:bold;}";

   this.sCF = new CheckBox(this.sigGB); this.sCF.text = "FWHM";
   this.sCF.checked = params.sigmaUseFWHM;
   this.sCF.onCheck = function(c){ params.sigmaUseFWHM = c; };
   this.sCE = new CheckBox(this.sigGB); this.sCE.text = _T("W_ECC").replace(":","");
   this.sCE.checked = params.sigmaUseEccentricity;
   this.sCE.onCheck = function(c){ params.sigmaUseEccentricity = c; };
   this.sCS = new CheckBox(this.sigGB); this.sCS.text = "SNR";
   this.sCS.checked = params.sigmaUseSNR;
   this.sCS.onCheck = function(c){ params.sigmaUseSNR = c; };
   this.sCN = new CheckBox(this.sigGB); this.sCN.text = _T("W_NOISE").replace(":","");
   this.sCN.checked = params.sigmaUseNoise;
   this.sCN.onCheck = function(c){ params.sigmaUseNoise = c; };
   this.sCT = new CheckBox(this.sigGB); this.sCT.text = _T("W_STARS").replace(":","");
   this.sCT.checked = params.sigmaUseStars;
   this.sCT.onCheck = function(c){ params.sigmaUseStars = c; };

   this.scR = new HorizontalSizer; this.scR.spacing = 16;
   this.scR.add(this.sCF); this.scR.add(this.sCE); this.scR.add(this.sCS);
   this.scR.add(this.sCN); this.scR.add(this.sCT); this.scR.addStretch();

   this.sigGB.sizer = new VerticalSizer;
   this.sigGB.sizer.margin  = 8;
   this.sigGB.sizer.spacing = 5;
   this.sigGB.sizer.add(this.ssR);
   this.sigGB.sizer.addSpacing(4);
   this.sigGB.sizer.add(this.sigML);
   this.sigGB.sizer.add(this.scR);
   ...
   this.cPsf.addItem("Gaussian"); this.cPsf.addItem("Moffat 4");
   this.cPsf.addItem("Moffat 6"); this.cPsf.addItem("Moffat 8");
   this.cPsf.currentItem     = params.psfType;
   this.cPsf.onItemSelected  = function(i){ params.psfType = i; };

   this.spLay = mkSp(this.psfGB, _T("STRUCT_LAYERS"), COL_PLABEL_W, params.structureLayers, 1,8,0,42);
   this.spLay.onValueUpdated = function(v){ params.structureLayers = Math.round(v); };
   this.spSen = mkSp(this.psfGB, _T("SENSITIVITY"),   COL_PLABEL_W, params.sensitivity,      0.01,1,2,52);
   this.spSen.onValueUpdated = function(v){ params.sensitivity = v; };
   this.spMS  = mkSp(this.psfGB, _T("MAX_STARS"),     COL_PLABEL_W, params.maxStars,          10,5000,0,52);
   this.spMS.onValueUpdated  = function(v){ params.maxStars = Math.round(v); };
   this.spUL  = mkSp(this.psfGB, _T("PSF_UPPER_LIMIT"), COL_PLABEL_W, params.upperLimit,        0,1,2,52);
   this.spUL.onValueUpdated  = function(v){ params.upperLimit = v; };

   this.p1 = new HorizontalSizer; this.p1.spacing = 4;
   this.p1.add(this.lbPsf); this.p1.add(this.cPsf); gap(this.p1);
   this.p1.add(this.spLay); this.p1.addStretch();
   this.p2 = new HorizontalSizer; this.p2.spacing = 4;
   this.p2.add(this.spSen); gap(this.p2); this.p2.add(this.spMS); gap(this.p2);
   this.p2.add(this.spUL); this.p2.addStretch();

   this.psfGB.sizer = new VerticalSizer;
   this.psfGB.sizer.margin  = 8;
   this.psfGB.sizer.spacing = 5;
   this.psfGB.sizer.add(this.p1);
   this.psfGB.sizer.add(this.p2);
   ...
   var metricKeys = ["fwhm","fwhmPx","eccentricity","snr","starCount",
                     "noise","median","qualityScore","starResidual"];
   ...
   // Double-clicking a row opens the corresponding image file
   this.resultsTree.onNodeDoubleClicked = function( node, col )
   {
      Console.writeln( "Table double-clicked on row: " + node.text(0) );
      var rowIdx = parseInt( node.text(0), 10 ) - 1;
      if ( rowIdx >= 0 && rowIdx < params.results.length )
      {
         var r = params.results[rowIdx];
         if ( r && r.filePath )
         {
            try
            {
   ...
   this.statsTextBox.setMaxHeight(220);
   this.statsTextBox.styleSheet  =
      "QTextEdit{font-family:monospace;font-size:10px;background:#1e1e1e;color:#e0e0e0;}";
   ...
   // Folder score panel
   this.folderScoreGB = new GroupBox(this);
   this.folderScoreGB.title = _T("FOLDER_SCORE_TITLE");
   this.folderScoreGB.setMinHeight( 65 );

   this.folderScoreLabel = new Label(this.folderScoreGB);
   this.folderScoreLabel.text          = _T("FOLDER_SCORE_EMPTY");
   this.folderScoreLabel.textAlignment = TextAlign_Left | TextAlign_VertCenter;
   this.folderScoreLabel.styleSheet    =
      "QLabel{font-family:monospace;font-size:10px;color:#e0e0e0;}";

   this.folderScoreGB.sizer = new VerticalSizer;
   this.folderScoreGB.sizer.margin  = 6;
   this.folderScoreGB.sizer.spacing = 2;
   this.folderScoreGB.sizer.add(this.folderScoreLabel);
   ...
   this.niBtn.onMousePress = function()
   {
      params.save();
      this.hasFocus = true;
      this.pushed   = false;
   ...
   {
      if ( !params.inputDirectory.length )
         { (new MessageBox(_T("MSG_NO_DIR"),TITLE,StdIcon_Error,StdButton_Ok)).execute(); return; }
      var d  = params.inputDirectory;
      if ( !hasTrailingSlash(d) ) d += "/";
      var lp = d + params.rejectedDirName + "/" + SSP_RESTORE_LOG;
      if ( !File.exists(lp) )
         { (new MessageBox(_T("MSG_NO_RESTORE_LOG"),TITLE,StdIcon_Warning,StdButton_Ok)).execute(); return; }
      if ( (new MessageBox(_T("MSG_RESTORE_CONFIRM"),TITLE,StdIcon_Question,
                            StdButton_Yes,StdButton_No)).execute() === StdButton_Yes )
      {
         var e = new SSPEngine(params);
         var c = e.restoreRejectedFiles();
         (new MessageBox(_TF("MSG_RESTORED_N",c),TITLE,StdIcon_Information,StdButton_Ok)).execute();
      }
   };
   ...
   this.runBtn.onClick = function()
   {
      if ( !params.inputDirectory.length )
         { (new MessageBox(_T("MSG_NO_DIR"),TITLE,StdIcon_Error,StdButton_Ok)).execute(); return; }
      if ( !File.directoryExists(params.inputDirectory) )
         { (new MessageBox(_T("MSG_DIR_NOT_FOUND")+"\n\n"+params.inputDirectory,
                           TITLE,StdIcon_Error,StdButton_Ok)).execute(); return; }
      ...
      if ( isDryRun )
      {
         ...
         try
         {
            ...
            dlg.progressLabel.text   = format("%s --- %d frames",
                                              _T("PROGRESS_DONE"), params.results.length);
   ...
   this.bR.add(this.niBtn);
   this.bR.add(this.restBtn);
   this.bR.add(this.rstBtn);
   this.bR.addStretch();
   this.bR.add(this.runBtn);
   this.bR.add(this.canBtn);
   ...
   this.rightPanel.add(this.folderScoreGB);
   ...
   this.dualPanel.add(this.rightPanel, 100);
   ...
   // updateResultsPanels()
   //
   // Refreshes the graph control, results table, folder score panel and
   // statistics panel after a completed analysis run.
   // ====================================================================
   this.updateResultsPanels = function( results )
   {
      ...
      for ( var i = 0; i < results.length; ++i )
      {
         ...
         var col;
         if      ( !r.analysisOK ) col = 0xFF9E9E9E;
         else if ( r.rejected )    col = 0xFFF44336;
         else                      col = 0xFF4CAF50;
         for ( var c = 0; c < 11; ++c ) node.setTextColor(c, col);

         // Tooltip on the row: rejection reasons if any
         if ( r.rejectionReasons && r.rejectionReasons.length > 0 )
            node.setToolTip( 10, r.rejectionReasons.join("\n") );
      }

      // Folder score panel
      dlg.updateFolderScorePanel( params.folderScore );
      ...
      // Inner helper: compute descriptive statistics for an array of numbers
      function calcStats( arr )
      {
         if ( arr.length === 0 ) return { min:0, max:0, mean:0, median:0, stddev:0, cv:0 };
         ...
         return { min:mn, max:mx, mean:mean, median:med, stddev:sd, cv:cv };
      }

      // Collect accepted and rejected values separately for comparative stats
      var accFwhm=[], rejFwhm=[], accEcc=[], rejEcc=[];
      var accSnr=[], rejSnr=[], accStar=[], rejStar=[];
      var accNoise=[], rejNoise=[], accMed=[], rejMed=[];
      var accScore=[], allScore=[];

      for ( var i = 0; i < valid.length; ++i )
      {
         var r = valid[i];
         allScore.push( r.qualityScore );
         if ( r.rejected )
         {
            rejFwhm.push(r.fwhm); rejEcc.push(r.eccentricity);
            rejSnr.push(r.snr);   rejStar.push(r.starCount);
            rejNoise.push(r.noise); rejMed.push(r.median);
         }
         else
         {
            accFwhm.push(r.fwhm); accEcc.push(r.eccentricity);
            accSnr.push(r.snr);   accStar.push(r.starCount);
            accNoise.push(r.noise); accMed.push(r.median);
            accScore.push(r.qualityScore);
         }
      }

      function fmtRow( name, s, prec )
      {
         return format( "%-14s min=%-9s max=%-9s mean=%-9s med=%-9s \u03C3=%-8s CV=%.1f%%",
                        name,
                        s.min.toFixed(prec), s.max.toFixed(prec),
                        s.mean.toFixed(prec), s.median.toFixed(prec),
                        s.stddev.toFixed(prec), s.cv );
      }
      ...
      var txt = format( "Frames: %d total  |  %d accepted  |  %d rejected  |  %d failed  " +
                        "(rejection rate: %.1f%%)\n",
                        results.length, acc, rej, fail,
                        results.length > 0 ? (rej+fail)*100.0/results.length : 0 );
      txt += hr;
      txt += "ACCEPTED frames:\n";
      txt += fmtRow( "  FWHM (\")",     calcStats(accFwhm),  3 ) + "\n";
      txt += fmtRow( "  Eccentricity", calcStats(accEcc),   4 ) + "\n";
      txt += fmtRow( "  SNR",          calcStats(accSnr),   2 ) + "\n";
      txt += fmtRow( "  Star Count",   calcStats(accStar),  0 ) + "\n";
      txt += fmtRow( "  Noise",        calcStats(accNoise), 6 ) + "\n";
      txt += fmtRow( "  Median",       calcStats(accMed),   5 ) + "\n";
      txt += fmtRow( "  Score",        calcStats(accScore), 4 ) + "\n";

      if ( rej > 0 )
      {
         txt += hr;
         txt += "REJECTED frames:\n";
         txt += fmtRow( "  FWHM (\")",     calcStats(rejFwhm),  3 ) + "\n";
         txt += fmtRow( "  Eccentricity", calcStats(rejEcc),   4 ) + "\n";
         txt += fmtRow( "  SNR",          calcStats(rejSnr),   2 ) + "\n";
         txt += fmtRow( "  Star Count",   calcStats(rejStar),  0 ) + "\n";
         txt += fmtRow( "  Noise",        calcStats(rejNoise), 6 ) + "\n";
         txt += fmtRow( "  Median",       calcStats(rejMed),   5 ) + "\n";
      }
   ...
   // ====================================================================
   // updateFolderScorePanel()
   //
   // Renders the folder quality score details in the dedicated panel.
   // fs is the object returned by SSPEngine.computeFolderScore() or null.
   // ====================================================================
   this.updateFolderScorePanel = function( fs )
   {
      if ( !fs )
      {
         dlg.folderScoreLabel.text = _T("FOLDER_SCORE_EMPTY");
         return;
      }

      // Grade colour coding via stylesheet
      var gradeColor;
      switch ( fs.grade )
      {
         case "A": gradeColor = "#4CAF50"; break;
         case "B": gradeColor = "#8BC34A"; break;
         case "C": gradeColor = "#FFC107"; break;
         case "D": gradeColor = "#FF9800"; break;
         default:  gradeColor = "#F44336"; break;
      }

      var txt = format(
         "Grade: %s   Score: %.4f (%.1f%%)     " +
         "Accepted: %d / %d   Rejected: %d   Failed: %d\n" +
         "Acceptance: %.1f%%   Frame score: %.4f   Consistency: %.4f   " +
         "FWHM component: %.4f   SNR component: %.4f",
         fs.grade, fs.folderScore, fs.folderScore * 100,
         fs.acceptedFrames, fs.analyzedFrames, fs.rejectedFrames, fs.failedFrames,
         fs.acceptanceRatio * 100, fs.meanScoreNorm, fs.consistencyNorm,
         fs.fwhmNorm, fs.snrNorm
      );

      dlg.folderScoreLabel.text       = txt;
      dlg.folderScoreLabel.styleSheet =
         "QLabel{font-family:monospace;font-size:10px;color:" + gradeColor + ";}";
   };
   ...
   this.syncUI = function()
   {
      ...
      dlg.cbFwhm.checked     = params.useFWHM;
      dlg.spFMin.setValue(params.fwhmMin);
      dlg.spFMax.setValue(params.fwhmMax);
      dlg.cbEcc.checked      = params.useEccentricity;
      dlg.spEcc.setValue(params.eccentricityMax);
      dlg.cbSnr.checked      = params.useSNR;
      dlg.spSnr.setValue(params.snrMin);
      dlg.cbRes.checked      = params.useStarResidual;
      dlg.spRes.setValue(params.starResidualMax);
      dlg.cbStar.checked     = params.useStars;
      dlg.spStar.setValue(params.starsMin);
      dlg.cbNoise.checked    = params.useNoise;
      dlg.spNoise.setValue(params.noiseMax);
      dlg.cbMed.checked      = params.useMedian;
      dlg.spMedMin.setValue(params.medianMin);
      dlg.spMedMax.setValue(params.medianMax);
      dlg.keepSlider.setValue(params.keepPercentage);
      dlg.wF.setValue(params.weightFWHM);
      dlg.wE.setValue(params.weightEccentricity);
      dlg.wS.setValue(params.weightSNR);
      dlg.wT.setValue(params.weightStars);
      dlg.wN.setValue(params.weightNoise);
      dlg.spSL.setValue(params.sigmaLow);
      dlg.spSH.setValue(params.sigmaHigh);
      dlg.sCF.checked  = params.sigmaUseFWHM;
      dlg.sCE.checked  = params.sigmaUseEccentricity;
      dlg.sCS.checked  = params.sigmaUseSNR;
      dlg.sCN.checked  = params.sigmaUseNoise;
      dlg.sCT.checked  = params.sigmaUseStars;
      dlg.cPsf.currentItem = params.psfType;
      dlg.spLay.setValue(params.structureLayers);
      dlg.spSen.setValue(params.sensitivity);
      dlg.spMS.setValue(params.maxStars);
      dlg.spUL.setValue(params.upperLimit);
      dlg.syncMode();
   };
