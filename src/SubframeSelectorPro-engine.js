...
// - Folder quality score computed from accepted/rejected frame statistics
...
function SSPEngine( params )
{
   ...
   function assignNativeMetrics( m, result )
   {
      ...
      if ( m.length !== undefined && typeof m !== "string" )
      {
         ...
         if ( result.fwhmPx <= 0 )
            result.fwhmPx = arrayValue( m, 4, 0 ) > 0 && arrayValue( m, 4, 0 ) < 50
                            ? arrayValue( m, 4, 0 ) : arrayValue( m, 6, 0 );
         if ( result.eccentricity <= 0 )
            result.eccentricity = arrayValue( m, 7, 0 ) < 1 ? arrayValue( m, 7, 0 ) : result.eccentricity;
         if ( result.starCount <= 0 )
            result.starCount = Math.round( arrayValue( m, 14, 0 ) > 0
                                           ? arrayValue( m, 14, 0 ) : arrayValue( m, 11, 0 ) );
         if ( result.starResidual <= 0 )
            result.starResidual = arrayValue( m, 13, 0 ) > 0
                                  ? arrayValue( m, 13, 0 ) : arrayValue( m, 12, 0 );
      ...
      result.fwhmPx = toNumber( firstDefined( m, ["fwhmPx","fwhmPixels","fwhm","FWHM"], result.fwhmPx ),
                                result.fwhmPx );
      result.eccentricity = toNumber( firstDefined( m, ["eccentricity","ecc","Eccentricity"],
                                                    result.eccentricity ), result.eccentricity );
      result.snr = toNumber( firstDefined( m, ["snrWeight","snr","SNRWeight","SNR","weight","Weight"],
                                           result.snr ), result.snr );
      result.starResidual = toNumber( firstDefined( m, ["starResidual","StarResidual","residual","Residual",
                                                        "residualMean"], result.starResidual ),
                                      result.starResidual );
      result.starCount = Math.round( toNumber( firstDefined( m, ["stars","Stars","starCount","nStars",
                                                                  "starSupport","StarSupport"],
                                                              result.starCount ), result.starCount ) );
      result.noise = toNumber( firstDefined( m, ["noise","Noise","noiseEstimate","noiseSupport","NoiseSupport"],
                                             result.noise ), result.noise );
      result.median = toNumber( firstDefined( m, ["median","Median","medianSignal"], result.median ),
                                result.median );
      ...
      for ( var k in m )
      {
         ...
         else if ( result.starCount <= 0 && kl.indexOf( "star" ) >= 0 &&
                   (kl.indexOf( "count" ) >= 0 || kl.indexOf( "support" ) >= 0 || kl === "stars") && n > 0 )
   ...
   this.getFileSignature = function( filePath )
   {
      try
      {
         ...
         var ts = fi.lastModified ? fi.lastModified.getTime().toString() : "0";
         return sz.toString() + "_" + ts;
      }
      catch (e)
      {
         try
         {
            return File.size( filePath ).toString() + "_0";
         }
         catch (e2)
         {
            return "unknown_" + filePath;
         }
      }
   };
      ...
      for ( var i = 0; i < rs.length; ++i )
      {
         ...
         if ( this.params.useFWHM )
         {
            if ( r.fwhm > this.params.fwhmMax )
               { r.rejected = true; r.rejectionReasons.push(
                  format( "FWHM %.2f > %.2f", r.fwhm, this.params.fwhmMax ) ); }
            if ( r.fwhm < this.params.fwhmMin )
               { r.rejected = true; r.rejectionReasons.push(
                  format( "FWHM %.2f < %.2f", r.fwhm, this.params.fwhmMin ) ); }
         }
         if ( this.params.useEccentricity && r.eccentricity > this.params.eccentricityMax )
            { r.rejected = true; r.rejectionReasons.push(
               format( "Ecc %.3f > %.3f", r.eccentricity, this.params.eccentricityMax ) ); }
         if ( this.params.useSNR && r.snr < this.params.snrMin )
            { r.rejected = true; r.rejectionReasons.push(
               format( "SNR %.1f < %.1f", r.snr, this.params.snrMin ) ); }
         if ( this.params.useStarResidual && r.starResidual > this.params.starResidualMax )
            { r.rejected = true; r.rejectionReasons.push(
               format( "Residual %.4f > %.4f", r.starResidual, this.params.starResidualMax ) ); }
         if ( this.params.useStars && r.starCount < this.params.starsMin )
            { r.rejected = true; r.rejectionReasons.push(
               format( "Stars %d < %d", r.starCount, this.params.starsMin ) ); }
         if ( this.params.useNoise && r.noise > this.params.noiseMax )
            { r.rejected = true; r.rejectionReasons.push(
               format( "Noise %.6f > %.6f", r.noise, this.params.noiseMax ) ); }
         if ( this.params.useMedian )
         {
            if ( r.median < this.params.medianMin )
               { r.rejected = true; r.rejectionReasons.push(
                  format( "Median %.4f < %.4f", r.median, this.params.medianMin ) ); }
            if ( r.median > this.params.medianMax )
               { r.rejected = true; r.rejectionReasons.push(
                  format( "Median %.4f > %.4f", r.median, this.params.medianMax ) ); }
         }
      }
   };
   ...
   this.applyPercentageRejection = function( rs )
   {
      ...
      var keep = Math.max( 1, Math.min( v.length,
                           Math.round( v.length * this.params.keepPercentage / 100 ) ) );
      for ( var i = keep; i < v.length; ++i )
      {
         v[i].rejected = true;
         v[i].rejectionReasons.push( format( "Score %.4f --- below top %d%% (rank %d/%d)",
                                             v[i].qualityScore, this.params.keepPercentage,
                                             i + 1, v.length ) );
      }
   };
      ...
      for ( var i = 0; i < v.length; ++i )
      {
         ...
         for ( var k in metrics )
         {
            ...
            if ( m.hiw )
            {
               if ( z > sH )
                  { r.rejected = true; r.rejectionReasons.push(
                     format( "%s: %.4f is %.1f\u03C3 above mean %.4f", k, r[k], z, m.mean ) ); }
               if ( z < -sL && k === "fwhm" )
                  { r.rejected = true; r.rejectionReasons.push(
                     format( "%s: %.4f is %.1f\u03C3 below mean %.4f", k, r[k], Math.abs(z), m.mean ) ); }
            }
            else
            {
               if ( z < -sL )
   ...
   // computeFolderScore()
   //
   // Computes a composite quality score for the entire session folder based
   // on the ratio of accepted frames, their metric statistics relative to the
   // full set, and the spread of individual quality scores.  The result is
   // stored in params.folderScore and returned as a plain object with all
   // contributing sub-scores for display.
   //
   // Score components (each in [0,1]):
   //   acceptanceRatio - fraction of analysed frames that passed rejection
   //   meanScoreNorm   - mean per-frame quality score of accepted frames
   //   consistencyNorm - 1 minus the coefficient of variation of accepted scores
   //                     (lower spread = higher consistency)
   //   fwhmNorm        - mean FWHM of accepted frames normalised over full set
   //   snrNorm         - mean SNR of accepted frames normalised over full set
   //
   // Final score = weighted average of the five components, range [0,1].
   // ========================================================================
   this.computeFolderScore = function( results )
   {
      var accepted = [], all = [];
      for ( var i = 0; i < results.length; ++i )
      {
         ...
         if ( !r || !r.analysisOK ) continue;
         all.push( r );
         if ( !r.rejected ) accepted.push( r );
      }

      var out = {
         totalFrames:    results.length,
         analyzedFrames: all.length,
         acceptedFrames: accepted.length,
         rejectedFrames: all.length - accepted.length,
         failedFrames:   results.length - all.length,
         acceptanceRatio: 0,
         meanScoreNorm:   0,
         consistencyNorm: 0,
         fwhmNorm:        0,
      ...
      // Acceptance ratio component
      out.acceptanceRatio = accepted.length / all.length;
      ...
      // Mean per-frame quality score of accepted frames (already in [0,1])
      var scoreSum = 0;
      for ( var i = 0; i < accepted.length; ++i )
         scoreSum += accepted[i].qualityScore;
      var meanScore = scoreSum / accepted.length;
      out.meanScoreNorm = meanScore;

      // Consistency: 1 - CV of accepted quality scores, clamped to [0,1]
      var scoreArr = [];
      for ( var i = 0; i < accepted.length; ++i )
         scoreArr.push( accepted[i].qualityScore );
      var sdScore = this._stddev( scoreArr, meanScore );
      var cv = (meanScore > 0) ? (sdScore / meanScore) : 1;
      out.consistencyNorm = Math.max( 0, Math.min( 1, 1 - cv ) );

      // FWHM component: lower accepted mean FWHM relative to full-set mean is better
      var allFwhm = [], accFwhm = [];
      for ( var i = 0; i < all.length; ++i )      allFwhm.push( all[i].fwhm );
      for ( var i = 0; i < accepted.length; ++i ) accFwhm.push( accepted[i].fwhm );
      var meanFwhmAll = this._mean( allFwhm );
      var meanFwhmAcc = this._mean( accFwhm );
      // Accepted mean FWHM <= overall mean is ideal (score 1); worse degrades toward 0
      out.fwhmNorm = (meanFwhmAll > 0)
                     ? Math.max( 0, Math.min( 1, meanFwhmAll / Math.max( meanFwhmAll, meanFwhmAcc ) ) )
                     : 0.5;

      // SNR component: higher accepted mean SNR relative to full-set mean is better
      var allSnr = [], accSnr = [];
      for ( var i = 0; i < all.length; ++i )      allSnr.push( all[i].snr );
      for ( var i = 0; i < accepted.length; ++i ) accSnr.push( accepted[i].snr );
      var meanSnrAll = this._mean( allSnr );
      var meanSnrAcc = this._mean( accSnr );
      out.snrNorm = (meanSnrAll > 0)
                    ? Math.max( 0, Math.min( 1, meanSnrAcc / Math.max( meanSnrAll, meanSnrAcc ) ) )
                    : 0.5;

      // Weighted combination of components
      var wA = 30, wS = 30, wC = 20, wF = 10, wN = 10;
      var total = wA + wS + wC + wF + wN;
      out.folderScore = (
         out.acceptanceRatio * wA +
         out.meanScoreNorm   * wS +
         out.consistencyNorm * wC +
         out.fwhmNorm        * wF +
         out.snrNorm         * wN
      ) / total;

      out.folderScore = Math.max( 0, Math.min( 1, out.folderScore ) );

      // Assign letter grade
      var pct = out.folderScore * 100;
      if      ( pct >= 90 ) out.grade = "A";
      else if ( pct >= 75 ) out.grade = "B";
      else if ( pct >= 60 ) out.grade = "C";
      else if ( pct >= 40 ) out.grade = "D";
      else                  out.grade = "F";

      // Store in params for external access
      this.params.folderScore = out;
   ...
   this.moveRejectedFiles = function()
   {
      ...
      if ( !File.directoryExists( rejDir ) )
      {
         try
         {
            File.createDirectory( rejDir, false );
            Console.writeln( "Created: " + rejDir );
         }
         catch ( e )
         {
            if ( File.directoryExists( rejDir ) || File.exists( rejDir ) )
            {
            ...
            else
            {
               try
               {
                  ...
                  Console.writeln( "Created (recursive): " + rejDir );
               }
               catch ( e2 )
               {
                  throw e2;
   ...
   this.exportCSV = function()
   {
      ...
      try
      {
         ...
         // Append folder score summary as comment block at the end of the CSV
         if ( this.params.folderScore )
         {
            var fs = this.params.folderScore;
            lines.push( "" );
            lines.push( "# Folder Quality Score Summary" );
            lines.push( format( "# Grade: %s   Score: %.4f", fs.grade, fs.folderScore ) );
            lines.push( format( "# Accepted: %d / %d   Rejected: %d   Failed: %d",
                                fs.acceptedFrames, fs.analyzedFrames,
                                fs.rejectedFrames, fs.failedFrames ) );
            lines.push( format( "# Acceptance ratio: %.1f%%   Mean frame score: %.4f",
                                fs.acceptanceRatio * 100, fs.meanScoreNorm ) );
            lines.push( format( "# Consistency: %.4f   FWHM component: %.4f   SNR component: %.4f",
                                fs.consistencyNorm, fs.fwhmNorm, fs.snrNorm ) );
         }
   ...
   this.printReport = function()
   {
      ...
      // Print per-folder score summary
      if ( this.params.folderScore )
      {
         var fs = this.params.folderScore;
         Console.writeln( "\n Folder Quality Assessment:" );
         Console.writeln( format( "  Grade: %s   Overall score: %.4f (%.1f%%)",
                                  fs.grade, fs.folderScore, fs.folderScore * 100 ) );
         Console.writeln( format( "  Acceptance ratio : %.1f%%   (component: %.4f)",
                                  fs.acceptanceRatio * 100, fs.acceptanceRatio ) );
         Console.writeln( format( "  Mean frame score : %.4f", fs.meanScoreNorm ) );
         Console.writeln( format( "  Score consistency: %.4f   FWHM: %.4f   SNR: %.4f",
                                  fs.consistencyNorm, fs.fwhmNorm, fs.snrNorm ) );
      }
   ...
   this.batchAnalyzeNative = function( filesToAnalyze, totalFiles )
   {
      ...
      catch ( e )
      {
         ...
         Console.warningln( " Batch analysis failed: " + e.message +
   ...
   this.run = function()
   {
      ...
      Console.writeln( "PSF backend: "        + (this.params.useNativeSS
                                                  ? "Native SubframeSelector" : "StarDetector (fallback)") );
      ...
      var cacheData = null, cacheHits = 0;
      ...
      for ( var i = 0; i < cachedResults.length; ++i )
      {
         ...
         if ( this.progressCallback )
            this.progressCallback( cachedResults[i].index + 1, files.length,
                                   cachedResults[i].result.fileName );
      }
         ...
         if ( this.params.useNativeSS && filesToAnalyze.length > 1 )
         {
            ...
            for ( var i = 0; i < filesToAnalyze.length; ++i )
            {
               ...
               if ( this.params.results[idx] === null )
               {
                  if ( !warnedFallback )
                  {
                     Console.warningln( " Native batch returned unrecognized measurements for some frames;" +
                                        " using StarDetector fallback." );
                     warnedFallback = true;
                  }
         ...
         else
         {
            for ( var i = 0; i < filesToAnalyze.length; ++i )
            {
               ...
               Console.writeln( format( "[%d/%d] %s",
                                        idx + 1, files.length,
                                        _TF("PROGRESS_FRAME",
                                            File.extractName( filesToAnalyze[i].path )) ) );
      ...
      // Compute per-frame quality scores over all valid results
      var validAll = [];
      for ( var i = 0; i < this.params.results.length; ++i )
      ...
      // Compute folder-level score after rejection is finalised
      this.computeFolderScore( this.params.results );
   ...
   this._modeName = function()
   {
      switch ( this.params.rejectionMode )
      {
         ...
         case SSP_MODE_SIGMA:      return format( _T("MODE_SIGMA") + " (\u03C3L=%.1f \u03C3H=%.1f)",
                                                  this.params.sigmaLow, this.params.sigmaHigh );
